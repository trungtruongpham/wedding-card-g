  ...
  return (
    <>
      ...
      <button
        ...
        aria-label={failed ? "Không thể phát nhạc" : playing ? "Tạm dừng nhạc" : "Phát nhạc"}
        title={failed ? "Không thể tải bản nhạc. Kiểm tra đường dẫn trong wedding-data.js." : weddingData.music.title}
      >
        <span aria-hidden="true" className={`absolute inset-1 rounded-full border border-white/20 ${playing ? "animate-music-spin" : ""}`}>
          <span className="absolute left-1/2 top-1/2 size-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/85" />
        </span>
        {playing ? (
          <span aria-hidden="true" className="relative flex h-4 items-center gap-0.5">
            {["55%", "100%", "42%", "78%"].map((height) => (
              <span key={height} className="music-bar block w-1 rounded-full bg-white" style={{ height }} />
            ))}
          </span>
        ) : (
          <svg aria-hidden="true" className="relative size-5" viewBox="0 0 24 24" fill="currentColor">
