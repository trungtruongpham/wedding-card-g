export const weddingData = {
  couple: {
    groom: {
      shortName: "Vũ Văn Tuấn",
      fullName: "Vũ Văn Tuấn",
      familyRole: "",
    },
    bride: {
      shortName: "Lê Hương Giang",
      fullName: "Lê Hương Giang",
      familyRole: "",
    },
  },
  families: [
    {
      label: "Ông Bà",
      parents: ["Vũ Văn Dũng", "Phạm Thị Doan"],
      address: "Tổ dân phố Trung Thu, phường Trường Thi, tỉnh Ninh Bình",
      weeddingTime: "18:30 - Thứ Bảy",
      weddingDay: "19/09/2026",
      lunarWeddingDay: "Tức 09/08 Âm lịch",
    },
    {
      label: "Ông Bà",
      parents: ["Lê Xuân Trường", "Trần Thị An"],
      address: "Số 4/75 An Phong, TP Nam Định, Tỉnh Ninh Bình",
      weeddingTime: "10:00 - Chủ Nhật",
      weddingDay: "20/09/2026",
      lunarWeddingDay: "Tức 10/08 Âm lịch",
    },
  ],
  ceremony: {
    time: "13:00",
    weekday: "Chủ Nhật",
    day: "20",
    month: "09",
    year: "2026",
    lunarDate: "Tức ngày 10 tháng 08 năm Bính Ngọ",
    location: "Tư gia nhà trai",
  },
  reception: {
    startsAt: "2026-09-19T18:00:00+07:00",
    endAt: "2026-09-20T18:00:00+07:00",
    durationHours: 2,
    weekday: "Thứ Bảy",
    guestArrival: "17:30",
    dinnerTime: "18:00",
    lunarDate: "Tức ngày 15 tháng 11 năm Ất Tỵ",
  },
  venue: {
    name: "Trung Tâm Hội Nghị White Palace",
    address: "194 Hoàng Văn Thụ, Phường 9, Quận Phú Nhuận, TP. Hồ Chí Minh",
  },
  dressCode: {
    note: "Trang phục dự tiệc",
    colors: ["#511419", "#8c3a3f", "#c9a24a", "#ece4d8"],
  },
  timeline: [
    { time: "17:00", label: "Đón khách" },
    { time: "18:00", label: "Khai tiệc", icon: "/assets/timeline-camera.webp" },
    {
      time: "18:30",
      label: "Nghi thức cưới",
      icon: "/assets/timeline-cake.webp",
    },
    {
      time: "19:00",
      label: "Cắt bánh & nâng ly",
      icon: "/assets/timeline-toast.webp",
    },
    { time: "20:30", label: "Kết thúc tiệc" },
  ],
  gallery: [
    "/assets/gallery-01.jpg",
    "/assets/gallery-02.jpg",
    "/assets/gallery-03.jpg",
    "/assets/gallery-04.jpg",
    "/assets/gallery-05.jpg",
    "/assets/gallery-06.jpg",
    "/assets/gallery-07.jpg",
  ],
  wishes: [
    {
      name: "Duy Khang",
      message: "Chúc mừng ngày vui của hai bạn, trăm năm hạnh phúc bền lâu!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Lan Chi",
      message: "Đẹp đôi quá! Chúc hai bạn sống bên nhau đầu bạc răng long.",
      time: "12:30 26/07/2026",
    },
    {
      name: "Tuấn Anh",
      message:
        "Mừng hạnh phúc hai bạn! Chúc gia đình nhỏ luôn đầy ắp tiếng cười.",
      time: "12:30 26/07/2026",
    },
    {
      name: "Khánh Vy",
      message: "Chúc cô dâu chú rể luôn giữ được nụ cười này mãi mãi nhé!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Gia Bảo",
      message:
        "Nhìn thiệp là thấy tình yêu rồi. Chúc hai bạn trăm năm viên mãn!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Phương Anh",
      message: "Chúc đám cưới thật trọn vẹn và ấm áp. Hạnh phúc nhé hai bạn!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Minh Đức",
      message:
        "Cuối cùng cũng tới ngày trọng đại, chúc mừng cặp đôi xứng lứa vừa đôi!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Thu Hà",
      message: "Chúc hai bạn mãi mãi yêu thương và bên nhau trọn đời!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Hoàng Long",
      message:
        "Mẫu thiệp đẹp quá, tông đỏ đô sang trọng ghê. Chúc mừng hai bạn!",
      time: "12:30 26/07/2026",
    },
    {
      name: "Ngọc Trâm",
      message:
        "Chúc Tuấn Kiệt và Minh Anh trăm năm hạnh phúc, sớm sinh quý tử nhé!",
      time: "12:30 26/07/2026",
    },
  ],
  giftAccounts: [
    {
      id: "groom",
      label: "Chú Rể - Vũ Văn Tuấn",
      bank: "Techcombank",
      accountNumber: "1711968386",
      accountHolder: "Vũ Văn Tuấn",
      qrImage: "/assets/qr-chu-re.jpg",
    },
    {
      id: "bride",
      label: "Cô Dâu - Hương Giang",
      bank: "Viettinbank",
      accountNumber: "108884554959",
      accountHolder: "Lê Hương Giang",
      qrImage: "/assets/qr-co-dau.jpg",
    },
  ],
  music: {
    title: "Em đồng ý (I do)",
    url: "https://cdn.chungdoi.com/music/em-dong-y-i-do.mp3",
  },
};
